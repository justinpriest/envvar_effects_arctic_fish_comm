...
spprichplot.biwk <- ggplot(predspeciesrich %>% group_by(biweekly) %>% 
  ...
  scale_y_continuous(limits = c(13, 18.5), breaks = 13:18, oob=scales::rescale_none) +
  theme_bw() +
...
spprichplot.year <- ggplot(predspeciesrich %>% group_by(Year) %>% 
  ...
  scale_y_continuous(limits = c(13, 18.5), breaks = 13:18) +
  theme_bw() +
  theme(text=element_text(family="Times New Roman", size=12), 
        axis.text.x = element_text(angle = 35, hjust = 1), 
...
spprichplots
#ggsave("../plotexports/Fig_spprichness2plot.png", plot = spprichplots,  width = 6, height = 4, units = "in")
